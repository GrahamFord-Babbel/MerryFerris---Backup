...
public class Waypoint : MonoBehaviour
{
    ...
    void Update()
    {
        infoStopWatch = infoStopWatch + Time.deltaTime;
        if (infoStopWatch > 75.0)
        {
            myInfo.SetActive(false);
        }
    ...
    public void Click()
    {
        ...
        Camera.main.transform.parent.transform.position = gameObject.transform.position + new Vector3(0,2,0);
        infoStopWatch = 0;
        AudioSource audio = GetComponent<AudioSource>();
        audio.Play();
        //myInfo = GameObject.Find("InfoTablet");
